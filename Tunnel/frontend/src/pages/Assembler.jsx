...
function Assembler() {
  ...
  const handleCheckAllSegmentsAreAvaliable = async (clickedContentFileSegments) => {
    try {
      const neededSegments = await CheckIfAllSegmentAreAvaliable(clickedContentFileSegments);
      setNeededFileSegments(neededSegments);
      setOpen(true);
      if (neededSegments && neededSegments.length !== 0) {
        alert("All segments are not available");
      }
    } catch (error) {
      console.error("Error checking segments:", error);
      alert("Error checking segments");
    }
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full z-50">
          {contents.map((content, index) => (
            <Contents
              key={index}
              item={content}
              onClick={() => handleCheckAllSegmentsAreAvaliable(content.fileSegments)}
            />
          ))}
        </div>
      </div>
            ...
            {building ? (
              <>
                <DialogTitle className="flex flex-col items-center justify-center">
                  <img
                    src={buildingDone ? CompleteGif : DownloadingGif}
                    alt="downloading..."
                  />
                  <p className="font-bold text-2xl text-gray-500">
                    {buildingDone ? "Download Completed!" : "Downloading..."}
                  </p>
                </DialogTitle>
                <DialogFooter>
                  <Button
                    className={`${
                      buildingDone ? "bg-green-500" : "bg-red-700"
                    } text-white cursor-pointer ${
                      buildingDone ? "hover:bg-green-600" : "hover:bg-red-800"
                  ...
                  >
                    {buildingDone ? "Go to Dashboard" : "Close"}
                  </Button>
                </DialogFooter>
              </>
            ) : (
              <>
                <DialogTitle>
                  {/* Optionally show selectedContent info here */}
                </DialogTitle>
                <DialogHeader>
                  {neededFileSegments && neededFileSegments.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-2 p-4">
                      <h1 className="font-bold">All segments are present!</h1>
                    </div>
                  )}
                </DialogHeader>
                <DialogFooter>
